...
class SandboxThrowableJavaTest {
    ...
    class TestMultiCatchExceptions : TestAsserter {
        override fun assertResult(testResult: ByteArray) {
            val result = StringList.parseFrom(testResult).valuesList
            assertThat(result[0]).isEqualTo("sandbox.com.r3.conclave.integrationtests.djvm.sandboxtests.MyExampleException:1")
            assertThat(result[1]).isEqualTo("sandbox.com.r3.conclave.integrationtests.djvm.sandboxtests.MyOtherException:2")
            assertThat(result[2]).isEqualTo("sandbox.com.r3.conclave.integrationtests.djvm.sandboxtests.BigTroubleException -> 3")
    ...
    class TestSuppressedUserExceptions : TestAsserter {
        override fun assertResult(testResult: ByteArray) {
            ...
            assertThat(result[1]).isEqualTo("sandbox.com.r3.conclave.integrationtests.djvm.sandboxtests.MyExampleException -> THROW: Hello World!")
            assertThat(result[2]).isEqualTo("sandbox.com.r3.conclave.integrationtests.djvm.sandboxtests.BigTroubleException -> BadResource: Hello World!")
        }
    }
}