...
public class pidigits {
    ...
    public static void main(String args[]) {
        ...
        // Conclave: Just build Pi into a string
        StringBuilder pi = new StringBuilder();

        while (n > 0) {
            if (n >= L){
                for (int i=0; i<L; i++) pi.append(digits.next());
                j += L;
            } else {
                for (int i=0; i<n; i++) pi.append( digits.next() );
                for (int i=n; i<L; i++) pi.append(" ");
                j += n;
            }
            pi.append("\t:"); pi.append(j);
            n -= L;
        }
    }
}
