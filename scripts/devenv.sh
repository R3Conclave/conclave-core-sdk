...
idea_version=${IDEA_VERSION:-"IC-223.7571.182"}
idea_download_file=ideaIC-2022.3.tar.gz
idea_download_address=https://download.jetbrains.com/idea/$idea_download_file
clion_version=${clion_version:-"2022.3"}
clion_download_file=CLion-2022.3.tar.gz
...
if [[ -z ${container_id} ]]; then
  ...
  if [ ${CONCLAVE_DOCKER_IDE:-""} = "1" ]; then
    if [ ! -d ${host_ide_dir}/idea-$idea_version ]; then
      ...
      echo "a3f53de8293b55739d916868d732bde521e33e59a5181e758d6f1691d479da9e  ${host_ide_dir}/$idea_download_file" | shasum -a 256 -c -
    ...
    if [ ! -d ${host_ide_dir}/clion-$clion_version ]; then
      ...
      echo "9b8870f37c16288e6bc6f654b561756271250f9e107299cd8da879c57ce5c5d9  ${host_ide_dir}/$clion_download_file" | shasum -a 256 -c -
