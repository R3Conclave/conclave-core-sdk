...
/*
 * A test class adapted from OpenJDK8/HotSpot class verification tests. The test cases
 * commented out are only relevant for checking the StackMapTable attribute, which is a 
 * feature not required by libverify.
 */
...
public class VerifyErrorTest {
    public static void main(String argv[]) throws Exception {
        for (Case c : VerifyErrorCases.cases) {
            ...
            if (c.isTestable()) {
                try {
                    URL testCasesJar = FileUtils.createInMemory("verifyErrorCases.jar");
                    ClassLoader cl = new URLClassLoader(new URL[] { testCasesJar });
