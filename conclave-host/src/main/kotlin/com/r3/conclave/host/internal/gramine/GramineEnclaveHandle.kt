...
class GramineEnclaveHandle(
    ...
    // was introduce in Java 9 but Enclave still supports Java 8.
    // That is why NULL_FILE was added in. The code was copied from ProcessBuilder.Redirect.DISCARD
    private val NULL_FILE: File = File(
        if (System.getProperty("os.name")
                .startsWith("Windows"))"NUL" else "/dev/null"
    )
    ...
    override fun initialise() {
        ...
        if (enclaveMode == EnclaveMode.RELEASE) {
            // Discard all the standard output and error generated by the enclave
            // when running in release mode
            gramineProcessBuilder.redirectError(NULL_FILE)
                                 .redirectOutput(NULL_FILE)
        } else {
            gramineProcessBuilder.inheritIO()
        }

        val gramineProcess = gramineProcessBuilder.start()
