...
abstract class AttestationTest {
    ...
    @Test
    fun `ensure the defined report data is present inside the signed quote that was generated by createAttestationQuote`() {
        enclaveHost.start(attestationParameters, null, null) { }

        // SHA-512 generates a 64-byte long array
        val reportDataOriginal = digest("SHA-512") { update("Test".toByteArray()) }
        val signedQuote = enclaveHost.createAttestationQuote(reportDataOriginal)
        val signedQuoteByteCursor = ByteCursor.wrap(SgxSignedQuote, signedQuote)

        val reportDataFromSignedQuote = signedQuoteByteCursor[quote][reportBody][reportData].bytes
        assertThat(reportDataFromSignedQuote).isEqualTo(reportDataOriginal)
    }
    ...
    fun `ensure an exception is raised by createAttestationQuote if the report data does not have the correct size`() {
        ...
        val exception = assertThrows<IllegalArgumentException> { enclaveHost.createAttestationQuote(emptyReportData) }
        assertThat(exception.message).isEqualTo("User data must be a 64-byte long byte array. User data size: 0")
    }
}
