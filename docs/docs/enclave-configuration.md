...
`./gradlew :<enclave-module>:build -PenclaveMode=(mock|simulation|debug|release)`
...
These values can then be used as part of an [enclave constraints](constraints.md) string.

The code hash and signer are also written to files in the enclave build directory at the following paths:

- Code hash: `<project>/<enclave-module-directory>/build/conclave/<enclave-mode>/mrenclave`
- Code signer: `<project>/<enclave-module-dir>/build/conclave/<enclave-mode>/mrsigner`
