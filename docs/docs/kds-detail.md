...
integrity of the KDS using remote attestation via the KDS `EnclaveInstanceInfo`.
...
SGX platform. The KDS can verify the `EnclaveInstanceInfo` report from the
...
is very similar to the `EnclaveConstraint` that a client application uses to
determine whether to trust an enclave or not. The enclave passes its
`EnclaveInstanceInfo` to the KDS along with the key specification for the key it
...
`EnclaveInstanceInfo`. This guarantees that only the recipient enclave can
...
attestation via its `EnclaveInstanceInfo` to prove its status to the KDS, and
